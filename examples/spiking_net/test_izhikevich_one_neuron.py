...
g_lsm_conn = conn_random.create_gaussian_matrix('g_lsm_conn', width, mean=10.0, std=0.2, sparsity=0.0, is_sparse=True)
...
current_rec = exp.get_group_cells_state("g_lsm", "g_lsm_rec")[:,0]
...
rec_values = np.array([round(current_rec[idx_plot_neuron],2)])
spike_values, = np.where(current_spike == 1)
scatter_values = np.full_like(spike_values, 0)
axs_0, = axs[0].plot(x_values, mem_values)
axs_1, = axs[1].plot(x_values, rec_values)
...
axs[1].set_title('Membrane recovery')
...
def updatefig(*args):
  global idx_anim, x_values, mem_values, rec_values, spike_values, scatter_values#, axs_1, axs_2

  #exp.run_step(feed_dict={input_lsm: np.random.randint(10, size=(input_size,1))})
  exp.run_step(feed_dict={input_lsm: np.full((input_size,1), 10)})
  ...
  if idx_anim >= plot_width:
    ...
    rec_values = np.concatenate((rec_values[1:], [current_rec[idx_plot_neuron]]))
    ...
    rec_values = np.concatenate((rec_values, [current_rec[idx_plot_neuron]]))
  ...
  axs_0.set_data(x_values, mem_values)
  axs_1.set_data(x_values, rec_values)
